...
const DashboardOverview = ({ user }) => {
  ...
  const fetchLeaderboard = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/auth/leaderboard`);
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/attendance/stats/teacher`, config);
      setStats(res.data);
  ...
  const fetchActiveCourseStudents = async (courseCode) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses/${courseCode}/students`, config);
