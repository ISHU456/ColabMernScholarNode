...
const QuizGenerator = ({ onClose, onSave, quizId }) => {
    ...
    if (quizId) {
      const fetchQuiz = async () => {
        ...
        try {
          ...
          const res = await axios.get(`${window.API_URL}/api/gamification/quizzes/${quizId}`, config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      if (quizId) {
        await axios.put(`${window.API_URL}/api/gamification/quizzes/${quizId}`, quizForm, config);
        alert("Neural Quiz Node Recalibrated!");
      } else {
        await axios.post(`${window.API_URL}/api/gamification/quizzes`, quizForm, config);
