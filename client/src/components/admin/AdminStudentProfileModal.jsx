...
const AdminStudentProfileModal = ({ studentId, user, onClose }) => {
        ...
        const fetchData = async () => {
            try {
                const endpoint = (user.role === 'admin' || user.role === 'hod') 
                    ? `${window.API_URL}/api/admin/students/${studentId}`
                    : `${window.API_URL}/api/auth/student-profile/${studentId}`;
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await axios.put(`${window.API_URL}/api/admin/students/${studentId}/enrollment`, {
