...
const AdminUserManagement = ({ user }) => {
    ...
    const fetchUsers = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${window.API_URL}/api/admin/users?role=${role}&dept=${selectedDept}&semester=${selectedSem}&section=${selectedSec}&isActive=${activeStatus}`, {
    ...
    const handleDelete = async (userId) => {
        ...
        try {
            ...
            const response = await axios.delete(`${window.API_URL}/api/admin/users/${userId}`, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
    ...
    const handleAuthorize = async (userId) => {
        try {
            await axios.post(`${window.API_URL}/api/admin/teachers/pending/${userId}/authorize`, {}, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
            fetchUsers();
        } catch (err) {
