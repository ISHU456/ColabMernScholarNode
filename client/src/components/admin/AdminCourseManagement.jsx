...
const AdminCourseManagement = ({ user }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [coursesRes, deptsRes] = await Promise.all([
        axios.get(`${window.API_URL}/api/admin/courses`, {
          headers: { Authorization: `Bearer ${user.token}` }
        }),
        axios.get(`${window.API_URL}/api/departments`)
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCourse) {
        await axios.put(`${window.API_URL}/api/admin/courses/${editingCourse._id}`, formData, {
          headers: { Authorization: `Bearer ${user.token}` }
        });
        showNotification('success', 'Course architecture synchronized successfully.');
      } else {
        await axios.post(`${window.API_URL}/api/admin/courses`, formData, {
          headers: { Authorization: `Bearer ${user.token}` }
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${window.API_URL}/api/admin/courses/${id}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
