...
const AdminQuizAttendees = ({ quizId, onClose, user }) => {
  ...
  const fetchAttendees = async () => {
    try {
      ...
      const res = await axios.get(`${window.API_URL}/api/gamification/quizzes/${quizId}/attendees`, config);
  ...
  const handleReset = async (userId) => {
     ...
     try {
        const config = { headers: { Authorization: `Bearer ${user.token}` } };
        await axios.delete(`${window.API_URL}/api/gamification/quizzes/${quizId}/attempts/${userId}`, config);
