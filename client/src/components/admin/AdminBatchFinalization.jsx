...
const AdminBatchFinalization = ({ user }) => {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${window.API_URL}/api/admin/courses`, {
  ...
  const fetchEligibleStudents = async () => {
    try {
      ...
      const { data } = await axios.get(`${window.API_URL}/api/admin/courses/eligible-students?courseId=${selectedCourse._id}&academicYear=${academicYear}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const handleFinalize = async () => {
    ...
    try {
        setLoading(true);
        const { data } = await axios.post(`${window.API_URL}/api/admin/courses/${selectedCourse._id}/finalize-batch`, {
