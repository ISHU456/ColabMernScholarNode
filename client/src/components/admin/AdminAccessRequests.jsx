...
const AdminAccessRequests = ({ user }) => {
  ...
  const fetchRequests = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/access-requests/pending`, {
  ...
  const handleResolve = async (id, status) => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/access-requests/resolve/${id}`, { status }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
