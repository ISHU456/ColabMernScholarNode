...
const Courses = () => {
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        const [coursesRes, deptsRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/courses`, {
            params: { departmentId: deptId, semester: activeSem === 'All' ? undefined : activeSem.split('-')[1] },
            headers: { Authorization: `Bearer ${user.token}` }
          }),
          axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/departments`)
        ]);
  ...
  useEffect(() => {
    const fetchAllCounts = async () => {
      ...
      for (const course of courses) {
        try {
          const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/auth/course-activity/${course.code}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/admin/courses`, formData, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      alert('New academic module established.');
      setShowModal(false);
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrixdeployment-server.onrender.com'}/api/courses`, {
