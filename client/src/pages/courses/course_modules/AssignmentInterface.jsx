...
const AssignmentInterface = ({ assignment, user, isTeacher, onBack, fetchAssignments }) => {
    ...
    const fetchSubmissions = async () => {
        try {
            const res = await axios.get(`${window.API_URL}/api/assignments/submissions/${assignment._id}`, {
    ...
    const handleDeleteSubmission = async (subId) => {
        ...
        try {
            await axios.delete(`${window.API_URL}/api/assignments/submissions/${subId}`, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
            fetchSubmissions();
