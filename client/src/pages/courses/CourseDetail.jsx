...
const CourseDetail = () => {
  ...
  const updateScheduleInDB = async (payload) => {
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses/${courseId}/schedule`, payload, {
  ...
  const handleTimetableImageUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses/${code}/schedule/image`, formData, {
        headers: { 
  ...
  const handleDeleteTimetableImage = async () => {
    ...
    try {
      const code = courseId.toUpperCase();
      await axios.put(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses/${code}/schedule`, { 
        timetableImageUrl: '' 
      }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const fetchProgress = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/progress/${courseId}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const handleStartLive = async (cId) => {
    if (isUserTeacher) {
      try {
        await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/notifications/broadcast-live`, { courseId: cId }, {
          headers: { Authorization: `Bearer ${user.token}` }
        });
      } catch (err) {
  ...
  const updateDbProgress = async (itemId, itemType) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/progress/update`, {
        courseId,
        itemId,
        itemType
      }, {
  ...
  const fetchCourseData = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses/${courseId}`, {
         headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  useEffect(() => {
    if (courseId && user?.token && hasIncrementedView.current !== courseId) {
       ...
       axios.patch(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses/${courseId}/view`, {}, {
         headers: { Authorization: `Bearer ${user.token}` }
  ...
  const fetchResources = async () => {
    try { 
      setIsLoading(true); 
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/resources?courseId=${courseId}`); 
  ...
  const fetchAssignments = async () => { 
    try { 
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/assignments/course/${courseId}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      }); 
  ...
  const fetchStudentSubmissions = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/assignments/my-submissions/${courseId}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const fetchAnnouncements = async () => { 
    try { 
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/announcements`); 
  ...
  const fetchOnlineCount = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/auth/course-activity/${courseId}`);
  ...
  const sendPulse = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/auth/pulse`, { courseId }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
    } catch (err) { 
      console.error(err); 
  ...
  const handleDelete = async (item) => { 
    ...
    try { 
      ...
      if (activeSection === 'assignments') {
        await axios.delete(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/assignments/${item._id}?courseId=${courseId}`, config);
        setAssignments(assignments.filter(a => a._id !== item._id));
      } else {
        await axios.delete(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/resources/${item._id}?courseId=${courseId}`, config); 
  ...
  const handlePreview = async (res) => {
    ...
    if (res.fileData) {
      url = `${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/resources/file/${res._id}`;
    }
