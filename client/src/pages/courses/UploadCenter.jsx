    ...
    const fetchData = async () => {
      try {
        const [courseRes, annRes] = await Promise.all([
          axios.get(`${window.API_URL}/api/courses/${courseId}`, {
             headers: { Authorization: `Bearer ${user.token}` }
          }),
          axios.get(`${window.API_URL}/api/announcements?courseId=${courseId}`)
        ]);
  ...
  const handleUploadCombined = async (e) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const res = await axios.post(`${window.API_URL}/api/resources/upload?courseId=${courseId}`, formData, {
          headers: { 
