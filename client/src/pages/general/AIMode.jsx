...
const AIMode = () => {
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get(`${window.API_URL}/api/chatbot/history`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const requestCredits = async () => {
    try {
      await axios.post(`${window.API_URL}/api/chatbot/request-credits`, {}, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      setRequestSent(true);
    } catch (err) {
  ...
  const deleteSession = async (sessionId) => {
    ...
    try {
      await axios.delete(`${window.API_URL}/api/chatbot/delete/${sessionId}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
  ...
  const grantCreditsToEmail = async () => {
    ...
    if (!email) return;
    try {
      await axios.post(`${window.API_URL}/api/chatbot/grant-by-email`, { email, amount }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
