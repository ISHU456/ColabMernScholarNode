...
const ResultVerification = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/courses`, {
  ...
  const handleReject = async () => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'https://scholarmatrix-api.onrender.com'}/api/results/reject`, { courseId, semester, academicYear, reason: rejectionReason }, { headers: { Authorization: `Bearer ${user.token}` } });
