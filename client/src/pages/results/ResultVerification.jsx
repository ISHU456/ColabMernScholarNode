...
const ResultVerification = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const { data } = await axios.get(`${window.API_URL}/api/courses`, {
  ...
  const handleReject = async () => {
    ...
    try {
      await axios.post(`${window.API_URL}/api/results/reject`, { courseId, semester, academicYear, reason: rejectionReason }, { headers: { Authorization: `Bearer ${user.token}` } });
