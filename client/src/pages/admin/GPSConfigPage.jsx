...
const GPSConfigPage = () => {
    ...
    const fetchConfig = async () => {
        try {
            ...
            const res = await axios.get('' + (import.meta.env.VITE_API_URL || 'https://colabmernscholarnodeserver.onrender.com') + ${import.meta.env.VITE_API_URL || 'https://colabmernscholarnodeserver.onrender.com'}/api/attendance/daily/gps-config', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const userStr = localStorage.getItem('user');
            const token = userStr ? JSON.parse(userStr).token : null;
            await axios.post('' + (import.meta.env.VITE_API_URL || 'https://colabmernscholarnodeserver.onrender.com') + ${import.meta.env.VITE_API_URL || 'https://colabmernscholarnodeserver.onrender.com'}/api/attendance/daily/gps-config', config, {
                headers: { Authorization: `Bearer ${token}` }
            });
